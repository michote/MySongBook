  ...
  function convLyrics (srceLyrics) {
    ...
    for (i=lyricsStartLine; i<lyricLines.length; i++) {
      if (!isVerseLine(lyricLines[i])) {  // blank or [x
        var langIdx = lyricLines[i].indexOf("lang=");
        var lang = "";
        if (langIdx !== -1) {
          langIdx = langIdx + 5;
          while (langIdx < lyricLines[i].length && lyricLines[i][langIdx] !== " ") {
            lang = lang + lyricLines[i][langIdx];
            langIdx++
          }
        }
        if (lyricLines[i] === "") {
      ...
      switch (propName) {
        case "author": case "title" : case "theme" :
          ...
          if (firstOfMulti) {
            ...
            var s1 = "    <\/" + propName + "s>";
            var s3 = "      <" + propName + ">" + propParam + "<\/" + propName + ">\n    <\/" + propName + "s>\n"
